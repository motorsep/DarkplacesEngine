...
char* Sys_ConsoleInput(void)
{
	...
	for (;; )
	{
		if (!GetNumberOfConsoleInputEvents(hinput, &numevents))
		{
			...
			if (err == 6)  // ERROR_INVALID_HANDLE - harmless timing issue after AllocConsole()
			{
				// Silently ignore and retry next frame - handle becomes valid soon
				numevents = 0;
			}
			else
			{
				cls.state = ca_disconnected;
				Sys_Error("Error getting # of console events (error code %x)", (unsigned int)err);
			}
		}
...
void Sys_InitConsole (void)
{
	...
	// if (cls.state == ca_dedicated)
#ifdef SERVERONLY
	if (1)  // Force dedicated console for SERVERONLY builds (darkplaces-dedicated.exe)
#else
