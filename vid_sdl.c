...
qboolean vid_supportrefreshrate = true; // SDL2 supports it
...
static SDL_Surface* vid_softsurface = NULL;

static int window_flags;

static cvar_t vid_fullscreen_exclusive = { CVAR_SAVE, "vid_fullscreen_exclusive", "0", "0 = desktop fullscreen (borderless window, best compatibility), 1 = exclusive fullscreen (true mode switch, better performance but worse alt-tab)" };
...
static void sdl_shutdown(void) { sdl_needs_restart = false; }
...
#endif
	Cvar_RegisterVariable(&vid_fullscreen_exclusive);

	if (SDL_Init(SDL_INIT_VIDEO) < 0)
		Sys_Error("Failed to init SDL video subsystem: %s", SDL_GetError());

	// Joystick init with correct reporting (from previous fix)
	if (SDL_InitSubSystem(SDL_INIT_JOYSTICK) < 0)
	{
		Con_Printf("Failed to init SDL joystick subsystem: %s\n", SDL_GetError());
		vid_sdl_initjoysticksystem = false;
	}
	else
	{
		Con_Printf("SDL joystick subsystem initialized successfully\n");
		vid_sdl_initjoysticksystem = true;
		SDL_JoystickEventState(SDL_ENABLE);
	}

	// Useful SDL2 hints
	SDL_SetHint(SDL_HINT_VIDEO_MINIMIZE_ON_FOCUS_LOSS, "0"); // Don't minimize on focus loss
	SDL_SetHint(SDL_HINT_MOUSE_RELATIVE_MODE_WARP, "1"); // Better relative mode
...
qboolean VID_InitModeGL(viddef_mode_t* mode)
{
	...
	win_half_width = mode->width / 2;
	win_half_height = mode->height / 2;
...
#ifndef USE_GLES2
	if (drivername && SDL_GL_LoadLibrary(drivername) < 0)
	{
	...
	if (mode->fullscreen)
	{
		if (vid_fullscreen_exclusive.integer)
	...
	if (!window)
	{
		Con_Printf("Failed to create window %ix%i: %s\n", mode->width, mode->height, SDL_GetError());
		return false;
	}
	...
	{
		Con_Printf("Failed to make OpenGL context current: %s\n", SDL_GetError());
	...
	// Manually load core OpenGL functions to guarantee they are valid even in core profile contexts
	qglGetString = (const GLubyte * (GLAPIENTRY*)(GLenum)) SDL_GL_GetProcAddress("glGetString");
	qglGetIntegerv = (void (GLAPIENTRY*)(GLenum, GLint*)) SDL_GL_GetProcAddress("glGetIntegerv");
	qglGetError = (GLenum(GLAPIENTRY*)(void)) SDL_GL_GetProcAddress("glGetError");

	if (!qglGetString || !qglGetIntegerv || !qglGetError)
	{
		Con_Printf("Failed to load one or more core OpenGL functions (glGetString/glGetIntegerv/glGetError)\n");
		SDL_GL_DeleteContext(context);
		context = NULL;
		SDL_DestroyWindow(window);
		window = NULL;
	...
	// Safe early test
	const char* test_vendor = (const char*)qglGetString(GL_VENDOR);
	if (test_vendor)
		Con_Printf("OpenGL context ready - Vendor: %s\n", test_vendor);
	else
		Con_Printf("OpenGL context ready (glGetString returned NULL - very old/broken driver?)\n");
	...
	VID_SetMouse(true, vid_mouse.integer, vid_mouse.integer && vid_stick_mouse.integer == 0);
...
void VID_Finish(void)
{
	...
	// Update active window state (vid_hidden and vid_hasfocus are set in Sys_SendKeyEvents)
	vid_activewindow = !vid_hidden && vid_hasfocus;

	// Update gamma every frame (cheap, and handles runtime changes)
	VID_UpdateGamma(false, 256);

	// Dynamic vsync handling (in case cvar changed at runtime)
	vid_usevsync = (vid_vsync.integer != 0);
	if (vid_usingvsync != vid_usevsync)
	{
		if (SDL_GL_SetSwapInterval(vid_usevsync ? 1 : 0) == 0)
			Con_DPrintf("Vsync %s\n", vid_usevsync ? "enabled" : "disabled");
		else
			Con_DPrintf("Failed to %s vsync: %s\n", vid_usevsync ? "enable" : "disable", SDL_GetError());
	...
	if (!vid_hidden)
	{
		// Optional GL finish for profiling/synchronization
